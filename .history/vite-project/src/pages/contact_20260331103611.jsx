...
function Contact() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Prepare email data for main notification to ADRIX CORE
    ...
    // Prepare auto-reply email data for client
    ...
    // SMS Message Content for ADRIX CORE Team
    const smsMessage = `🔔 ADRIX CORE Lead Alert!\n\nName: ${formData.name}\nEmail: ${formData.email}\nPhone: ${formData.phone || "Not provided"}\nProject: ${formData.projectType}\nBudget: ${formData.budget}\nMessage: ${formData.message.substring(0, 100)}${formData.message.length > 100 ? '...' : ''}\nTime: ${new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })}`;

    try {
      // Your EmailJS credentials
      const serviceId = "service_mxppefc";
      const adminTemplateId = "template_ewwsopp";      // Admin notification template (Contact Us)
      const autoReplyTemplateId = "template_cqwbt4h";   // Auto-reply template for client
      ...
      if (adminResponse.status === 200) {
        ...
        // 2. Send SMS to ADRIX CORE Team Numbers
        console.log("Sending SMS notifications to ADRIX CORE team...");
        
        // SMS via EmailJS using a simple email template (convert to SMS)
        // Method 1: Use EmailJS to send email that forwards to SMS via IFTTT/Zapier
        // Method 2: Direct SMS API (if you have Twilio, MSG91, etc.)
        
        // For now, we'll use console.log and you can integrate any SMS API
        // Replace this with actual SMS API call
        
        // Example: Using fetch to a webhook that sends SMS (like Make.com, Zapier, n8n)
        const webhookUrl = "https://hook.make.com/YOUR_WEBHOOK_ID"; // Replace with your webhook URL
        ...
        // Uncomment this when you have a webhook/SMS API
        /*
        try {
        ...
        */
        ...
        // Alternative: Send SMS as email to SMS gateway
        // This works with most Indian mobile carriers
        const smsGatewayEmails = [
          { number: "7447508006", email: "7447508006@jio.com" },    // Jio
          { number: "8080822156", email: "8080822156@jio.com" }     // Jio
        ];
        
        // Send SMS via email-to-SMS gateway
        for (const gateway of smsGatewayEmails) {
          const smsEmailParams = {
            to_email: gateway.email,
            from_name: "ADRIX CORE Lead",
          ...
          try {
            // You need to create an EmailJS template for SMS
            // await emailjs.send(serviceId, "template_sms", smsEmailParams);
            console.log(`SMS sent to ${gateway.number} via ${gateway.email}`);
          } catch (err) {
            console.log(`SMS gateway failed for ${gateway.number}`);
          }
        }
        ...
        console.log("Auto-reply EmailJS Response:", autoReplyResponse);
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", textAlign: "center" }}>
          ...
          >
            ...
            <p style={{ color: "rgba(224,247,255,0.6)", marginBottom: "2rem" }}>
              Schedule a 30-minute Google Meet. We'll discuss your project and answer any questions.
            </p>
            <div style={{
              background: "rgba(0,12,30,0.88)",
              border: `1px solid rgba(0,229,255,0.2)`,
              ...
              <p style={{ marginBottom: "1.5rem", color: "rgba(224,247,255,0.7)" }}>
                Select a time that works for you. We'll send a Google Meet link with calendar invite.
              </p>
              <motion.button
                whileHover={{ scale: 1.05, boxShadow: `0 0 30px rgba(0,229,255,0.4)` }}
                whileTap={{ scale: 0.95 }}
                onClick={() => window.open("https://calendar.google.com/calendar/u/0/appointments/schedules/AcZssZ3pLtQG2wR18uS9Y-3C7jK1mN4oP6qR8sT2uVwXyZ", "_blank")}
                style={{
                  background: `linear-gradient(135deg, ${C.cyan}, ${C.purple})`,
                  border: "none",
                  padding: "0.8rem 2rem",
