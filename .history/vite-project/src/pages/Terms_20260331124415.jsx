      ...
      <section ref={heroRef} style={{ 
        ...
        <motion.div style={{ opacity, scale }} className="hero-animate" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1 }}>
          <div style={{ textAlign: "center", maxWidth: 900, margin: "0 auto" }}>
            ...
            >
              Terms & <span style={{ color: C.cyan }}>Conditions</span>
            </motion.h1>
            <motion.p 
              initial={{ opacity: 0, y: 30 }}
              animate={{ opacity: 1, y: 0 }}
      ...
      <section style={{ padding: "40px 2rem 80px", position: "relative", zIndex: 2 }}>
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          {sections.map((s) => {
            ...
            return (
              <div key={s.id} className={`tnc-section ${isOpen ? "open" : ""}`}>
                <div className="tnc-section-header" onClick={() => toggleSection(s.id)}>
                  <span className="tnc-section-icon">{s.icon}</span>
                  <span className="tnc-section-num">{String(s.id).padStart(2, "0")}</span>
                  <span className="tnc-section-title">{s.title}</span>
                  <span className="tnc-chevron">▼</span>
                </div>

                {isOpen && (
                  <div className="tnc-section-body">
                    {s.isContact ? (
                      <div className="tnc-contact-grid">
                        {contactDetails.map((c, i) => (
                          <div className="tnc-contact-item" key={i}>
                            <div className="tnc-contact-label">{c.label}</div>
                            <div className="tnc-contact-value">
                              {c.isEmail ? (
                      ...
                      <>
                        {s.content && <p className="tnc-body-text">{s.content}</p>}
                        {s.list && (
                        ...
                        {s.listFooter && <p className="tnc-body-text">{s.listFooter}</p>}
                        {s.subsections && s.subsections.map((sub, i) => (
                          <div className="tnc-subsection" key={i}>
                            <div className="tnc-subsection-title">{sub.title}</div>
                            <p className="tnc-subsection-text">{sub.content}</p>
                          </div>
                        ))}
                      </>
                    )}
