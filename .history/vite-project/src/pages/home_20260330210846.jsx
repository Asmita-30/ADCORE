...
const CARD_POS = {
  "left":          { top: "30%", left: "2%", transform: "translateY(-50%)" },
  "right":         { top: "30%", right: "2%", transform: "translateY(-50%)" },
  "bottom-left":   { bottom: "15%", left: "5%" },
  "bottom":        { bottom: "2%", left: "50%", transform: "translateX(-50%)" },
  "bottom-right":  { bottom: "15%", right: "5%" },
  "right-bottom":  { bottom: "35%", right: "2%", transform: "translateY(50%)" },
};
...
const WIRE_ENDS = {
  ...
  "bottom-left":   [180, 580],
  "bottom":        [450, 620],
  "bottom-right":  [720, 580],
  "right-bottom":  [720, 420],
};
    ...
    <section ref={sectionRef} id="services" style={{ 
      ...
      <div style={{ 
        ...
        <svg viewBox="0 0 900 680" style={{ 
          ...
          {SERVICES.map(svc => {
            ...
            return (
              <g key={svc.pos}>
                ...
                {/* Animated particles on wires */}
                <circle r="5" fill={col} opacity="0.9">
                  <animateMotion 
                    dur={`${2 + Math.random() * 2}s`} 
                    repeatCount="indefinite" 
                    path={`M450,340 L${ex},${ey}`} 
                  />
                  <animate 
                    attributeName="r" 
                    values="3;6;3" 
          ...
          {/* Center Hub - Modern Design */}
          <g filter="url(#wg)">
            <circle cx="450" cy="340" r="45" fill="url(#grad1)" opacity="0.15" />
            <circle cx="450" cy="340" r="35" fill="rgba(0,229,255,0.1)" stroke={C.cyan} strokeWidth="1.5" />
            <circle cx="450" cy="340" r="25" fill="rgba(0,229,255,0.2)" />
            <text x="450" y="345" textAnchor="middle" fill={C.cyan} fontSize="12" fontFamily="'Orbitron',monospace" fontWeight="bold">
              CORE
            </text>
          </g>
        </svg>

        {/* Service Cards - Only Left, Right, and Bottom */}
        {SERVICES.map((svc, i) => (
          <motion.div
            key={svc.name}
            ...
            onMouseEnter={(e) => {
              ...
              e.currentTarget.style.boxShadow = `0 10px 30px ${svc.color}30`;
            }}
            onMouseLeave={(e) => {
          ...
          >
            ...
            <motion.div
              ...
              style={{
                position: "absolute",
                [svc.pos === "left" ? "right" : svc.pos === "right" ? "left" : "bottom"]: "-8px",
                [svc.pos === "left" ? "top" : svc.pos === "right" ? "top" : "left"]: "50%",
